...
For now, I loaded the data from the data file and clean the variable names. I then dropped the not as useful data variables and converted entry and vending variables from character variable to logical variable. 
...
```
##### There are `r count(filter(nycsubway_df_clean_distinct, ada == "TRUE"))` distinct stations.
...
#First convert route8-11 from num to character variables. which dataset do we use? distinct or no
...
```
##### There are `r count(filter(nycsubway_tidy_data, route_name == "A"))` served A train. There are `r count(filter(nycsubway_tidy_data, route_name == "A" & ada == "TRUE"))` stations served A train and are ADA compliant. 
...
The first file “pols-month” contains 822 observations of 9 variables originally related to the number of national politicians who are democratic or republican at any given time. After cleaning the data, the cleaned pols_df has `r nrow(pols_df)` rows and `r ncol(pols_df)` columns. Important variables in the dataset include year, month, president, and number of different parties on the associated dates. President variable indicates whether the president was democratic or republican. The year range in pols_df ranges from 1947 to 2015. The year range is the largest among all three data, so when merging the datasets, 1947 - 1948 have some NA values. 
...
The final_df is created by joining all three datasets by year and month. The final_df contains`r nrow(final_data)` rows and `r ncol(final_data)` columns. The year ranges from 1947 to 2015. 
